...
vector<Mat> tempOuts;
bool VERBOSE;
string NAME = "img.argb";

void postprocess(const vector<Mat>& out);
...
int32_t main(int32_t argc, char **argv) {
    ...
    if ( (0 == commandlineArguments.count("cid")) ||
        ...
        if (sharedMemory && sharedMemory->valid()) {
            ...
            //Mat frame,
            Mat blob;
            thread post (postHandle, sharedMemory);
...
void postHandle(void sharedMemory){
    while(1){
        if(postNotEmpty == 1){
            postprocess(tempOuts);
            postNotEmpty = 0;
            if (VERBOSE) {
...
void postprocess(const vector<Mat>& outs)
{
    vector<int> classIds;
